...
// --- [컴포넌트 1: 시장 상황 게이지] ---
const MarketGauge = ({ diffPoint, status, emoji, theme }) => {
  // 🚀 기획안 반영: -1200p(9시) ~ 0p(12시) ~ +650p(3시) 각도 매핑
  // (diffPoint + 1200) / (650 + 1200) * 180도 - 90도
  const calculateAngle = () => {
    let angle = ((diffPoint + 1200) / (650 + 1200) * 180) - 90;
    return Math.max(-90, Math.min(90, angle)); // 9시 ~ 3시 범위 고정
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', margin: '10px 0' }}>
      <svg width="240" height="130" viewBox="0 0 240 130">
        ...
        {/* 🚀 바늘 위치: 계산된 각도로 실시간 이동 */}
        <g transform={`translate(120, 120) rotate(${currentAngle})`}>
      ...
      <div style={{ marginTop: -30, textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: theme.subText, marginTop: 4 }}>NQ1! 변동: {diffPoint > 0 ? `+${diffPoint}` : diffPoint}p</div>
      </div>
    </div>
  );
};
...
export default function Home() {
  ...
  // 🚀 Admin 설정값 및 실시간 감시 로직 (여기서부터)
  const [adminSettings, setAdminSettings] = useState({ diffPoint: 0, status: "로딩중...", emoji: "⌛" });

  useEffect(() => {
    const settingsRef = doc(db, "settings", "market");
  ...
  const checkInApp = () => {
    ...
    return ua.includes("KAKAOTALK") || ua.includes("KAKAOWORK");
  };
  ...
  const handleLogin = async () => {
    if (checkInApp()) {
      alert("카톡/카카오워크 내에서는 구글 로그인이 차단됩니다.\n\n우측 상단 메뉴(⋮ 또는 🔗)를 눌러 ‘브라우저로 열기’로 접속해 주세요.");
      return;
    }
    try {
...
const nextPlan = buyPlan.find(p => p.turn === currentRound + 1);
  ...
  return (
    <>
      ...
      {/* 🚀 복잡한 감지 로직 제거하고 로그인 전이면 무조건 안내 텍스트 노출 */}
      {!user && (
        <div style={{ textAlign: 'center', padding: '15px 20px', backgroundColor: '#fff3cd', borderBottom: '1px solid #ffeeba' }}>
          <p style={{ color: '#856404', fontSize: '13px', fontWeight: 'bold', margin: 0, lineHeight: '1.6' }}>
            ⚠️ <span style={{color: '#ff453a'}}>카톡/인앱 브라우저</span>에서는 구글 로그인이 지원되지 않습니다.<br/>
            로그인이 안 될 경우, 우측 상단 메뉴를 눌러 <span style={{color: '#0a84ff'}}>[브라우저로 열기]</span>를 선택해 주세요.
          </p>
        </div>
      )}
        ...
        <div className="grid-controls">
          <div style={styles.gaugeSection}>
               <div style={{ textAlign: 'center', fontSize: 13, fontWeight: 'bold', color: theme.text, marginTop: 5 }}>
                   오늘 시장 구간: <span style={{color: '#3b82f6'}}>{adminSettings.status}</span>
               </div>
               {/* 🚀 이제 관리자가 Admin에서 설정한 DB 값이 바늘을 움직입니다. */}
               <MarketGauge 
                 diffPoint={adminSettings.diffPoint || 0} 
                 status={adminSettings.status} 
                 emoji={adminSettings.emoji} 
                 theme={theme} 
               />
          </div>
