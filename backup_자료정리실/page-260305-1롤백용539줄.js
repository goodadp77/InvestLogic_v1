...
export default function Home() {
  ...
  const [stockMaster, setStockMaster] = useState({}); // 가드 로직용 추가
  ...
  const updateStockSetting = (key, value) => {
    setStockSettings(prev => {
      const currentVal = prev[symbol] || { percent: "100", currentPrice: "30" };
  ...
  // 🥊 3. 안전한 종목 로드 로직 (index 필드 유무와 상관없이 무조건 노출)
  useEffect(() => {
    ...
    const unsubscribeStocks = onSnapshot(stocksRef, (snapshot) => {
      ...
      if (stockList.length > 0) {
        setStocks(stockList);
        setStockMaster(masterMap); // 🥊 어드민 마스터 가격 저장소 확보
  ...
  // 🥊 UX 지침 3 & 유지 로직: 종목 선택 시 자동 스크롤 및 로컬 저장
  ...
  // 🥊 [수정 로직] 매수 플랜 데이터 및 개선율 로직 복구
  const getPlanData = () => {
    const mySetting = stockSettings[symbol] || { percent: "100", currentPrice: "30" };
    ...
    return defaultRatios.map((percent, index) => {
      ...
      const improvement = index === 0 ? 0 : ((prevAvgPrice - avgPrice) / prevAvgPrice * 100);
        ...
        <div className="grid-controls">
          ...
          <div style={styles.section}>
            ...
            <div style={{...styles.controlItem, marginTop:10}}>
              ...
              <input type="text" value={mySetting?.currentPrice || ""} onChange={(e) => updateStockSetting('currentPrice', e.target.value)} style={styles.fullInput} placeholder="현재가 입력" />
            </div>
          </div>
        </div>
                    ...
                    return (
                      <div key={plan.turn} style={rowStyle}>
                        ...
                        <div style={{width:80, textAlign:'right', display:'flex', flexDirection:'column', alignItems:'flex-end'}}>
                          ...
                          {Number(plan.improvement) > 0 && !plan.isExecuted && <span style={{color: '#30d158', fontSize: 10, fontWeight: 'bold'}}>↓ {plan.improvement}% 개선</span>}
                        </div>
                      </div>
                    )
                  })}
