...
export default function Home() {
  ...
  // 🥊 UX 지침 3: 중앙 정렬을 위한 Ref 보관소
  const tabRefs = useRef({});

  // 🥊 어드민 DB 실시간 종목 로드
  useEffect(() => {
    const unsubscribeStocks = onSnapshot(collection(db, "stocks"), (snapshot) => {
      const stockList = snapshot.docs.map(doc => {
        const data = doc.data();
        return data.symbol ? data.symbol.toString() : (data.id ? data.id.toString() : doc.id.toString()); 
      });

      if (stockList.length > 0) {
        setStocks(stockList);
        setSymbol(prev => (stockList.includes(prev) ? prev : stockList[0]));
        
        setStockSettings(prev => {
          const newSettings = { ...prev };
          stockList.forEach(s => {
            if (!newSettings[s]) newSettings[s] = { percent: "100", currentPrice: "30" };
          });
          return newSettings;
        });
      }
  ...
  // 🥊 UX 지침 3: 종목 선택 시 자동 스크롤 기능 (Fixed View 구현)
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      try {
        ...
        if (interpSnap.exists()) { setInterpretation(interpSnap.data()); }
      } catch (error) {
  ...
  const getProInterpretation = () => {
    if (!interpretation || !marketStatus || !interpretation.comments) return null;
    const mKeyMap = { "극공포": "M1", "공포": "M2", "중립": "M3", "탐욕": "M4" };
    const mKey = mKeyMap[marketStatus];
    const dir = interpretation.direction;
    if (!interpretation.comments[mKey] || !interpretation.comments[mKey][dir]) return null;
    const activeVer = interpretation.comments[mKey][dir].active;
    return { dir, text: interpretation.comments[mKey][dir][activeVer] };
  };
  ...
  // 🥊 [조치 완료] 에러 타격: isProUser 선언 추가 (정의되지 않은 참조 에러 방지)
  const isProUser = userTier === "PRO" || userTier === "ADMIN";

  if (loading) return <div style={styles.loading}>⏳ 인증 확인 중...</div>;

  const proInfo = getProInterpretation();
        ...
        <div className="grid-controls">
          <div style={styles.gaugeSection}>
            ...
            <div style={{ textAlign: 'center', padding: '0 15px 20px 15px', marginTop: '-10px' }}>
              {isProUser ? (
                proInfo && (
                  <div style={{ fontSize: '13px', color: theme.subText, fontWeight: '500', lineHeight: '1.5' }} className="line-clamp-2">
                    <span style={{ color: proInfo.dir === "up" ? "#34c759" : "#ff3b30", fontWeight: 'bold' }}>
                      {proInfo.dir === "up" ? "▲ 상승 국면" : "▼ 하락 국면"}
                    </span>
          ...
          <div style={styles.section}>
            {/* 🥊 UX 지침 1 & 2: 가로 스크롤 레이아웃 (homestyles.js에서 힌트 스크롤바 관리됨) */}
            <div style={styles.tabContainer}>
              {stocks.map((t) => (
                <button 
                  key={t?.toString()} // 🥊 UX 지침 4: Math.random() 제거 (안정적 키값)
                  ref={el => tabRefs.current[t] = el} // 🥊 UX 지침 3: Ref 등록
                  onClick={() => handleSymbolChange(t)} // 🥊 UX 지침 3: 중앙 정렬 클릭 이벤트
            ...
            <div style={styles.controlGrid}>
              <div style={styles.controlItem}>
                ...
                <div style={{display:'flex', alignItems:'center'}}>
                  <input type="text" value={mySetting?.percent || "100"} onChange={(e) => { if (/^\d*$/.test(e.target.value)) updateStockSetting('percent', e.target.value); }} onBlur={(e) => { const cleaned = e.target.value.replace(/^0+(?=\d)/,''); updateStockSetting('percent', cleaned === "" ? "" : String(Number(cleaned))); }} style={styles.smallInput} />
            ...
            <div style={{...styles.controlItem, marginTop:10}}>
              ...
              <input type="text" value={mySetting?.currentPrice || "30"} onChange={(e) => { if (/^\d*$/.test(e.target.value)) updateStockSetting('currentPrice', e.target.value); }} onBlur={(e) => { const cleaned = e.target.value.replace(/^0+(?=\d)/,''); updateStockSetting('currentPrice', cleaned === "" ? "" : String(Number(cleaned))); }} style={styles.fullInput} placeholder="현재가 입력" />
            </div>
          </div>
        </div>
