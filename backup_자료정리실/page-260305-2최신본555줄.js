  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', margin: '10px 0' }}>
      ...
      <svg width="240" height="130" viewBox="0 0 240 130">
        ...
        <text x="35" y="125" fontSize="12" fill={theme.subText} textAnchor="middle" fontWeight="bold">공포</text>
        <text x="205" y="125" fontSize="12" fill={theme.subText} textAnchor="middle" fontWeight="bold">탐욕</text>
...
export default function Home() {
  ...
  const getProInterpretation = () => {
    ...
    return proInfo ? (proInfo.text || proInfo.A) : null;
  };

  const handleLogin = async () => { await socialLogin(); };
  const handleLogout = () => { signOut(auth); };

  // 🚀 [주입 5] 최신 엔진 기반 매수 플랜 계산 로직 (수치 100% 복구)
  const getPlanData = () => {
    // 🥊 기준가 우선순위 정립: 유저 현재가 > 어드민 마스터 현재가 > 0
    ...
    // 🥊 엔진 연동: calculateSplitPlan(userTier, marketRegime, totalBudget)
    ...
    if (confirm(`${symbol} ${p.turn}회차 기록하시겠습니까?`)) {
      try { 
        await addDoc(collection(db, "trades"), { 
          ...
          symbol: symbol, 
          type: "buy", 
          round: p.turn, // 🚀 round-turn 매핑 정합성 보강
