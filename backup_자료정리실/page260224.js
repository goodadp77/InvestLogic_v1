...
export default function Home() {
  ...
  const checkInApp = () => {
    ...
    // 안드로이드라면 크롬 브라우저로 자동 강제 실행 (영문 에러 원천 차단)
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        // 2. 🔒 로그인 완료 시에만 DB 감시 시작 (보안 규칙 준수)
        ...
        unsubscribeDb = onSnapshot(q, 
          ...
          (error) => {
            // 권한 에러 발생 시 로그만 남기고 에러 창 차단
            console.warn("Firestore Permission Note:", error.message);
          }
        );
      } else {
        // 🔓 비로그인 상태: DB 요청을 하지 않고 상태만 초기화
        setUserTier("FREE");
        setTradeHistory([]);
        if (unsubscribeDb) {
          unsubscribeDb();
  ...
  const handleLogin = async () => {
    if (checkInApp()) {
      // 자동 이동이 안 된 사용자(iOS 등)를 위해 더 명확한 경로 안내
...
<div style={styles.totalBar}>
     ...
     <span style={{
       fontSize: user ? 18 : 15, // 로그인 전에는 -3px 축소
       color: user ? '#30d158' : '#94A3B8', // 로그인 전에는 블루그레이(#94A3B8) 적용
       fontWeight: user ? 'bold' : 'normal' // 보조 안내 텍스트 느낌 유지
     }}>
...
{!user && (
    <div style={{ 
        textAlign: 'center', 
        backgroundColor: '#E2E8F0', // 라이트모드: 연한 블루그레이
        padding: '12px', 
        borderRadius: 8, 
        marginTop: 15, 
        border: '1px solid #CBD5E1', 
        cursor: 'pointer',
        // 다크모드 대응을 위한 인라인 스타일 (CSS 변수 사용 권장)
                    ...
                    {user && (
                        <div style={{ backgroundColor: 'rgba(255,255,255,0.5)', padding: '10px', borderRadius: 6, marginBottom: 10 }}>
                            <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 4 }}>
                                📊 무료 회원 예상 평단: <span style={{fontWeight: 'bold', color: theme.text}}>${finalFreeAvg.toLocaleString(undefined, {maximumFractionDigits:2})}</span>
                            </div>
                            <div style={{ fontSize: 12, color: '#8b5cf6', fontWeight: 'bold' }}>
                                ✨ PRO 전략 적용 시 예상 평단: ${proAvg.toLocaleString(undefined, {maximumFractionDigits:2})} (▼ 5.6% 추가 방어 효과)
                            </div>
                        </div>
                    )}

                    <div style={{ fontSize: 12, color: '#ef4444', fontWeight: 'bold', textAlign: 'right' }}>
                        {!user ? "👉 로그인 후 실제 전략 비교 가능" : "👉 PRO 전략 활성화하기"}
                    </div>
                </div>
            )}
