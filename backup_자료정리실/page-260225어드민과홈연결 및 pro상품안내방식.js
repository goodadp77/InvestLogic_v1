...
// --- [컴포넌트 1: 시장 상황 게이지] ---
// 요구사항: emoji 제거, upRate 기반 바늘 각도 계산 (0~100 clamp)
const MarketGauge = ({ status, upRate, theme }) => {
  // 바늘 각도 계산: upRate를 0~100 사이로 고정 후 -90도~90도로 변환
  const clampedRate = Math.min(Math.max(upRate, 0), 100);
...
// 요구사항: PC 중앙 정렬 (Centered Container 추가)
const TopNav = ({ user, handleLogin, handleLogout, theme }) => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  return (
    <div style={{ width: '100%', backgroundColor: theme.card, borderBottom: `1px solid ${theme.border}`, position: 'sticky', top: 0, zIndex: 50 }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          {user ? <button onClick={handleLogout} style={{ padding:'6px 12px', fontSize:12, backgroundColor: theme.bg, color: theme.text, border:`1px solid ${theme.border}`, borderRadius:4, cursor:'pointer' }}>로그아웃</button>
            : <button onClick={handleLogin} style={{ padding:'6px 15px', fontSize:12, backgroundColor:'#4285F4', color:'white', border:'none', borderRadius:4, fontWeight:'bold', cursor:'pointer' }}>
            로그인 (무료)
          </button>}
...
export default function Home() {
  ...
  // 어드민 연동 상태값
  const [marketStatus, setMarketStatus] = useState("2.반등 (상승초입)");
  ...
  // 하드코딩된 marketTemplates 제거 및 Firestore 연동 로직
  useEffect(() => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (diffPoint && low36) {
            ...
            setUpRate(calculatedRate);
            
            // 구간 판정
            if (calculatedRate < 25) setMarketStatus("1.저점 (바닥다지기)");
            else if (calculatedRate < 60) setMarketStatus("2.반등 (상승초입)");
            else if (calculatedRate < 100) setMarketStatus("3.고점 (급등구간)");
            else setMarketStatus("4.과열 (탐욕구간)");
          }
        }
      } catch (error) {
        console.error("Market settings load error:", error);
      }
    };
    fetchMarketSettings();
  }, []);
  ...
  const checkInApp = () => {
    ...
    const isInApp = ua.includes("kakaotalk") || ua.includes("kakaowork");
    if (isInApp && ua.includes("android")) {
      location.href = "intent://" + window.location.host + window.location.pathname + "#Intent;scheme=https;package=com.android.chrome;end";
    }
    return isInApp;
  };
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        unsubscribeDb = onSnapshot(q,
          (snapshot) => {
            setTradeHistory(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
          },
          (error) => { console.warn("Firestore Permission Note:", error.message); }
        );
      } else {
        setUserTier("FREE");
        setTradeHistory([]);
  ...
  const handleLogin = async () => {
    if (checkInApp()) {
      alert("보안 정책상 현재 브라우저에서는 구글 로그인이 불가능합니다.\n\n상단 또는 하단의 메뉴 버튼을 눌러 '다른 브라우저로 열기'를 선택해 주세요.");
      return;
    }
  ...
  // 분할계산 로직 유지 (기존 currentMarket 참조 부분 고정 비율로 대체)
  const getPlanData = () => {
    ...
    <>
      <style>{`
        ...
        @media (prefers-color-scheme: dark) {
          ...
          .login-guide-box span {
            color: #94A3B8 !important;
          }
          .login-guide-box span span {
      ...
      <div className="responsive-layout" style={{ fontFamily: '-apple-system, sans-serif' }}>
        ...
        <div className="grid-controls">
          <div style={styles.gaugeSection}>
            ...
            {/* MarketGauge에 status와 upRate 전달 */}
        ...
        <div className="grid-main">
          <div style={styles.section}>
            ...
            <div style={styles.totalBar}>
              <span style={{color: theme.text}}>총 매수 운영금</span>
              <span style={{
                fontSize: user ? 18 : 15,
                color: user ? '#30d158' : '#94A3B8',
                fontWeight: user ? 'bold' : 'normal'
              }}>
                {user ? `${totalInvested.toLocaleString()} 원` : "로그인 시 자동 계산"}
              </span>
            </div>
            ...
            {!user && (
              <div style={{
                ...
                cursor: 'pointer',
              }} onClick={handleLogin} className="login-guide-box">
                <span style={{color: '#475569', fontSize: 13, fontWeight: 'bold'}}>
                  🔒 로그인 시 <span style={{color: '#1E293B', textDecoration: 'underline'}}>매수 기록/저장/자동계산</span>이 활성화됩니다.
                </span>
              </div>
            )}
            ...
            {userTier !== 'PRO' && (
              <div style={{ backgroundColor: '#f5f3ff', padding: '18px', borderRadius: 8, marginTop: 15, border: '1px solid #ddd6fe', cursor: 'pointer' }} onClick={() => !user ? handleLogin() : alert('PRO 결제 안내 페이지로 이동합니다.')}>
                <div style={{ fontSize: 15, color: '#6d28d9', fontWeight: 'bold', marginBottom: 10 }}>💎 PRO 전략 엔진 활성화 🔒</div>
                <div style={{ fontSize: 13, color: '#4c1d95', marginBottom: 12, lineHeight: '1.6' }}>
                  • 하락 구간 자동 방어 전략 적용<br/>
                  • 상승 구간 비중 자동 최적화
                </div>
                {user && (
                  <div style={{ backgroundColor: 'rgba(255,255,255,0.5)', padding: '10px', borderRadius: 6, marginBottom: 10 }}>
                    <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 4 }}>📊 무료 회원 예상 평단: <span style={{fontWeight: 'bold', color: theme.text}}>${finalFreeAvg.toLocaleString(undefined, {maximumFractionDigits:2})}</span></div>
                    <div style={{ fontSize: 12, color: '#8b5cf6', fontWeight: 'bold' }}>✨ PRO 전략 적용 시 예상 평단: ${proAvg.toLocaleString(undefined, {maximumFractionDigits:2})} (▼ 5.6% 추가 방어 효과)</div>
                  </div>
                )}
                <div style={{ fontSize: 12, color: '#ef4444', fontWeight: 'bold', textAlign: 'right' }}>{!user ? "👉 로그인 후 실제 전략 비교 가능" : "👉 PRO 전략 활성화하기"}</div>
              </div>
            )}
